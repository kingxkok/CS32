...
//		swap(s[0], s[1]);
	...
	for (int i = 1; i < s.size(); i++)
		for (int j = i - 1; j >= 0 && comp(s[j + 1], s[j]); j--)
			swap(s[j + 1], s[j]);
...
int main()
{
	...
	// destruction time.
	{
		...
		vector<Student*> aSP;
		aSP.reserve(auxStudents.size());
		for (int i = 0; i < auxStudents.size(); i++) aSP.push_back(&auxStudents[i] );

		// TODO:  Sort the vector of pointers using the STL sort algorithm
		//        with compareStudentPtr as the ordering relationship.
		sort(aSP.begin(), aSP.end(), compareStudentPtr);
